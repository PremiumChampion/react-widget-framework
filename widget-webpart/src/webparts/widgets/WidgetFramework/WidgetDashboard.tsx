...
export const WidgetDashboard = (props: IGirdHostProps) =>
{
    ...
    // Apps and Widgets
    const [_widgets, _setWidgets] = React.useState(generateWidgetIndex(props.widgets));
    // Allow forced rerender
    // const [_update, _forceUpdate] = useState(true);
    // Keeps tract of column size changes
    const [_prevColCount, _setPrevColCount] = React.useState(0);
    // Is initial render
    const [_initialRender, _setInitialRender] = React.useState(true);
    // is resizing
    const [_resizing, _setResizing] = React.useState(false);
    // function to force an update
    const [_updateVersion, _forceUpdate] = React.useReducer(x => x + 1, 0);
    // contains the parents dimensions
    const { width } = React.useContext(ResizeContext);
    ...
    const createCorrectLayout = (columnCount: number, boundaryCollission: boolean, widgets?: BaseWidget[], lastItemId?: string) =>
    {
        ...
        // force update
        if (changes) _forceUpdate(undefined);
    };
    ...
    const renderPlaceholder = (layout: GridLayout.Layout[], oldItem: GridLayout.Layout, newItem: GridLayout.Layout, _placeholder: GridLayout.Layout, event: MouseEvent, element: HTMLElement) =>
    {
        ...
        if (!isNil(placeholder) && placeholder.childElementCount === 1)
        {
            const content = element.querySelector(".TS-PREVIEW");
            placeholder.insertAdjacentHTML('afterbegin', !isNil(content) ? content.outerHTML + "" : "");
        }
    };


    const GridLayoutProps: GridLayout.ReactGridLayoutProps = {
        cols: colCount,
        rowHeight: rowHeigth,
        width: width,
        isBounded: true,
    ...
    return (
        <div key={_updateVersion} style={{ overflow: "hidden" ,backgroundColor:"rgba(0,0,0,0)"}}>
            <GridLayout
                {...GridLayoutProps}
            >
                {renderWidgets()}
